...
Touch-first UI: bottom tab bar (`Veprime | Transfer | Produkte | Histori | Permbledhje`), bottom sheets, and card lists. For setup, auth, and API client see [README.md](README.md). For desktop modals and tables see [README-DESKTOP.md](README-DESKTOP.md).
...
**Dynamic Histori tab:** Lloji + Lokacioni chip filters, date row, advanced filters panel, same `MobileHistoriActionCard` list (location/transfer routes), tap → `DynamicHistoriBatchDetail` with full-screen edit (location sheets for route). Desktop opens history via **Historiku** on the action card (`DynamicHistoryModal` modal); mobile uses the Histori tab.
...
| --- | --- | --- |
| **Legacy** | `src/mobile/tabs/PermbledhjeTab.tsx` | `MobileDateRangeInput` (**Nga** / **Deri**), Kosovo and Albania cards with Hyrje/Dalje sasi and vlerë, **Shkarko Excel** |
| **Dynamic** | `tabs/DynamicPermbledhjeTab.tsx` | `MobileDateRangeInput`, one card per summary location (emoji + name) with all four metrics, **Shkarko Excel** |
